...
def process_func(path: str, aug_rate=1, missing_ratio=0.1, p_obs=0.2, mecha='MAR', opt="logistic"): ## p_obs, mecha input을 상위 함수에 추가
    ...
    gt_masks = produce_NA(observed_values, p_miss=missing_ratio, p_obs=p_obs, mecha=mecha, opt=opt)
...
class tabular_dataset(Dataset):
    ...
    def __init__(
        self, eval_length=9, use_index_list=None, aug_rate=1, missing_ratio=0.1, seed=0, p_obs=0.2, mecha='MAR', opt="logistic" ###
    ):
        ...
        processed_data_path = (
            f"./data_housing/missing_ratio-{missing_ratio}_seed-{seed}.pk"
        )
        processed_data_path_norm = (
            f"./data_housing/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
        )

        if not os.path.isfile(processed_data_path):
            self.observed_values, self.observed_masks, self.gt_masks = process_func(
                dataset_path, aug_rate=aug_rate, missing_ratio=missing_ratio, p_obs=p_obs, mecha=mecha, opt=opt ###
            )
