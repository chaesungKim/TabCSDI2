...
class Tokenizer(nn.Module):
    def __init__(
        ...
        category_offsets = torch.tensor([0] + categories[:-1]).cumsum(0) # [0, 9, 17, 9, 15, 7, 6, 3, 42]을 누적합한거. 즉 [0,9,26,35,...]. 각 범주형변수 카테고리의 시작위치를 표현함.
        self.d_token = d_token
        self.register_buffer("category_offsets", category_offsets) # 파라미터는 아니지만 state_dict에 포함되는 것
        self.category_embeddings = nn.Embedding(sum(categories) + 1, self.d_token) # (전체 카테고리 수 + 1)개짜리 임베딩. / d_token: 임베딩 차원
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        x = self.weight.T * x_num # (8,6)*(64,1,6) => (64,8,6)

        if x_cat is not None:
            x = x[:, np.newaxis, :, :] # (64,1,8,6)
            x = x.permute(0, 1, 3, 2) # (B,C=1,L,K=1) (64,1,6,8)
            x = torch.cat(
                [x, self.category_embeddings(x_cat + self.category_offsets[None])], # x_cat: 3차원 텐서, self.category_offsets[None]: 2차원?
                dim=2,
            ) # 연속형과 범주형을 결합. x => (64,1,15,8)
        if self.bias is not None:
            x = x + self.bias[None]
...
class CSDI_base(nn.Module):
    ...
    # def get_randmask(self, observed_mask): # mecha, missing_ratio, p_obs, opt 를 받아야 함..
    #     rand_for_mask = torch.rand_like(observed_mask) * observed_mask # 랜덤 마스크를 주는데, observed_mask에서 missing인 건 그대로 missing 이도록
    #     rand_for_mask = rand_for_mask.reshape(len(rand_for_mask), -1) # 2차원으로 크기 맞추기

    #     # MCAR로 마스킹
    #     for i in range(len(observed_mask)): # len(observed_mask): 전체 데이터 row 개수
    #         sample_ratio = np.random.rand() # 마스킹할 데이터 비율: 랜덤한 수
    #         num_observed = observed_mask[i].sum().item() # observed_mask의 한 row에서 observed인 셀 개수
    #         num_masked = round(num_observed * sample_ratio) # 관찰된 데이터들 중에서 마스킹할 데이터 수 (sample_ratio에 따름)
    #         rand_for_mask[i][rand_for_mask[i].topk(num_masked).indices] = -1 # 큰 순서대로 num_masked개 만을 추출해 -1로 바꿈
    #     cond_mask = (rand_for_mask > 0).reshape(observed_mask.shape).float() # missing:0, observed:1

    #     return cond_mask # output: 1과 0으로 이루어진 (20000,15) array
    
    ###
    def get_randmask(self, observed_data_org, observed_mask):
        # batch의 observed_data, observed_mask: torch.Tensor?
    ...
    def forward(self, batch, is_train=1):
        ...
        if is_train == 0:
            ...
            cond_mask = self.get_randmask(observed_data_org, observed_mask) ### observed_data 추가
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            cond_mask = gt_mask # 모르는 척 하는거 + 진짜 모르는 거
            # target_mask: 1 for manually masked elements
            target_mask = observed_mask - cond_mask # 모르는 척 하는거
            side_info = self.get_side_info(observed_tp, cond_mask)
    ...
    ###
    def evaluate_all(self, batch, n_samples): ### for using full_dataloader
        (
            observed_data,
            observed_mask,
            observed_tp,
        ...
        with torch.no_grad():
            ...
            target_mask = 1 - observed_mask # 여기서는 진짜 모르는거 밖에 없음. 모르는척 하는거 여기선 아는 것들임.
            side_info = self.get_side_info(observed_tp, cond_mask)
            samples = self.impute(observed_data, cond_mask, side_info, n_samples)
...
class TabCSDI(CSDI_base):
    ...
    def process_data(self, batch):
        ...
        observed_data_org = observed_data ### tokenizer하기 전 형태 저장. 
        # print(observed_data.shape) ### [64,1,15]
