...
def process_func(path: str, cat_list, missing_ratio=0.1, encode=True, p_obs=0.2, mecha='MAR', opt="logistic"):
    ...
    if encode == True:
        ...
        with open("./data_housing_ft/transformed_columns.pk", "wb") as f: ## write? 
            pickle.dump([cont_list, num_cate_list], f)

        with open("./data_housing_ft/encoder.pk", "wb") as f: ## write?
...
class tabular_Dataset(Dataset):
    # eval_length should be equal to attributes number. -> 9 (독립변수 8, 종속변수 1)
    def __init__(self, eval_length=9, use_index_list=None, missing_ratio=0.1, seed=0, p_obs=0.2, mecha='MAR', opt="logistic"):
        self.eval_length = eval_length
        np.random.seed(seed)

        dataset_path = "./data_housing_ft/housing.csv"
        processed_data_path = (
            f"./data_housing_ft/missing_ratio-{missing_ratio}_seed-{seed}.pk"
        )
        processed_data_path_norm = f"./data_housing_ft/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"

        # self.cont_cols is only saved in .pk file before normalization.
        cat_list = [] ## 모두 연속형 변수, regression -> 범주형 변수 처리 방법이 의미 없음. ft만 해보자. 
...
def get_dataloader(seed=1, nfold=5, batch_size=16, missing_ratio=0.1, p_obs=0.2, mecha='MAR', opt="logistic"):
    ...
    processed_data_path_norm = (
        f"./data_housing_ft/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
    )
