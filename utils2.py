...
def MCAR_mask(X, m_ratio, m_cols):
    n, d = X.shape
...
def MAR_mask(X, m_ratio, m_cols):
    ...
    idxs_obs = np.array([x for x in range(d) if x not in m_cols])
    idxs_nas = np.array(m_cols)

    p = m_ratio * d / len(m_cols) # 컬럼별 missing 비율
...
def MNAR_mask_logistic(X, m_ratio, m_cols, exclude_inputs=True): # p, p_params =.3, exclude_inputs=True):
    """
    ...
    idxs_params = np.array([x for x in range(d) if x not in m_cols]) if exclude_inputs else np.arange(d)
    idxs_nas = np.array(m_cols) if exclude_inputs else np.arange(d)
    ...
    if exclude_inputs: ### nan 생성에 따로 input으로 들어간 애들은 MCAR로 mask 생성해준 것
        mask[:, idxs_params] = torch.rand(n, len(idxs_params)) < p

    return mask

def MNAR_self_mask_logistic(X, m_ratio, m_cols): # p):
    """
    ...
    ### The parameters of this logistic model are random.
    idxs_params = np.array([x for x in range(d) if x not in m_cols])
    idxs_nas = np.array(m_cols)
    p = m_ratio * d / len(m_cols)
