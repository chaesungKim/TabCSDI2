...
def process_func(path: str, cat_list, encode=True, 
    ...
    data.replace(0, 1, inplace=True) # for what..
    ...
    # observed_masks 생성
    observed_masks, true_ps = produce_NA(data_observed_values, mecha=mecha1, opt=opt, m_ratio=m_ratio1, m_cols=m_cols) # missing:1
    ...
    gt_masks, _ = produce_NA(new_observed_values2, mecha=mecha2, opt=opt, m_ratio=m_ratio2, m_cols=m_cols, seed=1) ### seed 추가.
    ...
    with open("./data_insurance/transform.pk", "wb") as f:
    ...
    cont_list = [x for x in range(7) if x not in cat_list] # insurance
    for k, v in zip(cont_list, cont_cols):
        col_dict[k] = [v]
    col_dict = {key: col_dict[key] for key in sorted(col_dict)}
    with open("./data_insurance/column_dict.pk", "wb") as f:
...
class tabular_dataset(Dataset):
    ...
    def __init__(self, eval_length=11, use_index_list=None, seed=0,
                 mecha1 = 'MAR', m_ratio1 = 0.2, m_cols=[0],
                 mecha2 = 'MAR', m_ratio2 = 0.2, opt="selfmasked"):
        self.eval_length = eval_length
        np.random.seed(seed)

        col_type = 'missing_cat' if m_cols ==[1,4,5] else 'missing_num' if m_cols == [0,2,3,6] else 'random'

        dataset_path = "./data_insurance/insurance.data"
        processed_data_path = (
            f"./data_insurance/{mecha1}-{mecha2}_{m_ratio1}_seed-{seed}_{col_type}.pk"
        )
        processed_data_path_norm = f"./data_insurance/{mecha1}-{mecha2}_{m_ratio1}_seed-{seed}_{col_type}_max-min_norm.pk"
...
def get_dataloader(seed=1, nfold=5, batch_size=16, 
    ...
    if not os.path.isfile(processed_data_path_norm):
        ...
        for index, k in enumerate(dataset.cont_cols):
            ...
            # print('obs_ind: ',obs_ind, ' temp: ',temp)
