...
def process_func(path: str, cat_list, missing_ratio=0.4, p_obs=0.5, mecha='MAR', opt="logistic"):
    def sortby(x):
        if isinstance(x, str):
            return ord(x[1])
        else:
    ...
    encoder.fit(data)
    new_df2 = encoder.transform(data)
...
class tabular_dataset(Dataset):
    ...
    def __init__(self, eval_length=39, use_index_list=None, missing_ratio=0.4, seed=0, p_obs=0.5, mecha='MAR', opt="logistic"):
        ...
        if not os.path.isfile(processed_data_path):
            (
                ...
                dataset_path, cat_list=cat_list, missing_ratio=missing_ratio, p_obs=p_obs, mecha=mecha, opt=opt ###
            )
