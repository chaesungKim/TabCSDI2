...
def process_func(path: str, cat_list, missing_ratio=0.1, encode=True, p_obs=0.2, mecha='MAR', opt="logistic"):
    data = pd.read_csv(path, header=None)
    data.replace(" ?", np.nan, inplace=True)

    # Swap columns - 연속형, 범주형 순서로 정렬
    ...
    #     miss_indices = np.random.choice( ## get random indices to mask
    ...
    # 여기까지 new_df, new_observed_values, observed_masks, gt_masks 열 개수 15개
...
class tabular_dataset(Dataset):
    def __init__(self, eval_length=111, use_index_list=None, missing_ratio=0.1, seed=0, p_obs=0.2, mecha='MAR', opt="logistic"):
...
def get_dataloader(seed=1, nfold=5, batch_size=16, missing_ratio=0.1, p_obs=0.2, mecha='MAR', opt="logistic"):
    dataset = tabular_dataset(missing_ratio=missing_ratio, seed=seed, p_obs=p_obs, mecha=mecha, opt=opt) ## create masked dataset
    ...
    if not os.path.isfile(processed_data_path_norm):
        ...
        for index, k in enumerate(dataset.cont_cols):
            ...
            ## training with non-missing values?
            obs_ind = dataset.observed_masks[train_index, k].astype(bool) ## from training set, get observed mask indices (True/False)
            temp = dataset.observed_values[train_index, k] ## from training set, get observed values
            max_arr[index] = max(temp[obs_ind]) ## from non-missing data in training set, get max value
            min_arr[index] = min(temp[obs_ind]) ## from non-missing data in training set, get min value
        ...
        for index, k in enumerate(dataset.cont_cols): ## continuous variables
            dataset.observed_values[:, k] = ( ## normalize observed values that used for training
    ...
    ## after normalization, split into train, val, test and make dataloader
